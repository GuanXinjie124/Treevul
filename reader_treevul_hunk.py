import json
from allennlp import dataimporttext_field_embedder
import numpy as np
...
from .process_edit import construct_diff_sequence

logger = logging.getLogger(__name__)  # pylint: disable=invalid-name

@DatasetReader.register("reader_treevul_hunk")
class ReaderTreeVul(DatasetReader):
    def __init__(self,
        ...
        self._edit_to_id = {"equal": 1, "delete": 2, "insert": 3, "replace": 4}  # 0 is the detault type id, we use it for special tokens and label embedding
        self._empty_token = "<empty>"
    ...
    def read_dataset(self, file_path):          #读取和处理数据集并返回
        ...
        for s in samples:
            ...
            for rem_diff, add_diff in zip(s["REM_DIFF"], s["ADD_DIFF"]):
                ...
                for rem_token, add_token, edit_token in edit_sequence:
                    edit_id = self._edit_to_id[edit_token.text]
                    if rem_token.text != self._empty_token:
                        rem_diff.append(dataclasses.replace(rem_token, type_id=edit_id))
                    if add_token.text != self._empty_token:
